...
- **Lyrics** — `/api/lyrics?q=` or `?artist=&title=`, backed by a free keyless lookup
- **Batch info** — `/api/youtube/info/batch?ids=a,b,c` resolves a whole queue concurrently
- **Related videos** — `/api/youtube/related?id=`
- **Thumbnails** — `/api/youtube/thumbnail?id=&size=small|medium|large|xl|max`
- **One-shot download** — `/api/youtube/download?id=&type=audio|video`, a real file instead of a live stream
- **Quality selector** — `/api/youtube/audio?quality=low|medium|high`
- **Usage stats** — `/api/stats` (request counts, cache hit rate), gated behind your API key
- **Optional cookies** — `YT_COOKIES` env var as a last-resort tier for the rare video that needs a real session
- **Optional webhooks** — `WEBHOOK_URL` fires on a resolve failure or when a `/playlist` request finishes
- **Per-key quotas** — `KEY_QUOTAS` caps daily requests for specific API keys you hand out
- **Landing page** — `/home` (docs-style overview; `/` stays the raw JSON keepalive bots rely on)
...
| Method | Path | Returns |
|---|---|---|
| `GET` | `/api/youtube/audio?id=<video_id>&quality=<optional>` | `{success, audio: {audio_streams: [...]}}` — signed URLs, IP-locked to this server. `quality=low\|medium\|high` reorders results toward that bitrate |
...
| `GET` | `/api/youtube/info/batch?ids=<a,b,c>` | Same as above for up to 50 ids at once, resolved concurrently |
| `GET` | `/api/youtube/related?id=<video_id>&limit=10` | "More like this" recommendations |
| `GET` | `/api/youtube/playlist?url=<playlist_url>&limit=50` | Playlist entries |
| `GET` | `/api/youtube/thumbnail?id=<video_id>&size=<small\|medium\|large\|xl\|max>` | Proxied thumbnail image bytes |
| `GET` | `/api/youtube/download?id=<video_id>&type=<audio\|video>&height=<optional>` | A fully-downloaded file (attachment), not a live stream — for re-upload use cases |
| `GET` | `/api/lyrics?q=<query>` or `?artist=&title=` | `{success, artist, title, lyrics}` |
| `GET` | `/api/stats` | Request counts, cache hit rate, resolve failures — gated behind your API key |
...
| `KEY_QUOTAS` | *(blank)* | Per-key daily caps: `key1:100,key2:500`; keys with no entry stay unlimited |
| `YT_COOKIES` | *(blank)* | Netscape-format cookies.txt **contents** (not a path) — last-resort tier for videos that need a real session |
| `WEBHOOK_URL` | *(blank)* | POSTed to on a resolve failure or when `/playlist` finishes — fire-and-forget, never blocks the caller |
