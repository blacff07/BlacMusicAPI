...
def _key_quotas(name: str, default: str = "") -> dict:
    """Parses 'key1:100,key2:500' into {'key1': 100, 'key2': 500}.
    Malformed entries are skipped rather than raising, since this runs at
    import time — a typo here shouldn't crash the whole app on boot."""
    out: dict = {}
    for part in os.getenv(name, default).split(","):
        part = part.strip()
        if not part or ":" not in part:
            continue
        key, _, limit_str = part.partition(":")
        key, limit_str = key.strip(), limit_str.strip()
        if not key or not limit_str.isdigit():
            continue
        out[key] = int(limit_str)
    return out
...
class Settings:
    ...
    # Optional per-key daily request caps: "key1:100,key2:500". A key with
    # no entry here (e.g. your own bot's key) stays unlimited — this is for
    # capping usage on keys you hand out to *other* people's bots, not a
    # general throttle. Resets ~24h after process start.
    KEY_QUOTAS: dict = _key_quotas("KEY_QUOTAS")
    ...
    # --- Optional failure webhook (opt-in, off by default) ---
    # Unset by default. When set, a terminal resolve failure (every ladder
    # rung exhausted) fires a fire-and-forget POST here instead of your bot
    # only finding out by polling/retrying. Never blocks or delays the
    # caller's own error response.
