...
async def _remux_hls_stream(result: StreamResult) -> StreamingResponse:
    ...
    HLS delivers AAC audio as raw ADTS frames inside its MPEG-TS segments.
    MP4 needs AAC in ASC (AudioSpecificConfig) form instead, so a plain
    `-c:a copy` produces a corrupt/truncated output — ffmpeg errors out
    mid-stream with "Malformed AAC bitstream detected". The
    `aac_adtstoasc` bitstream filter converts the framing losslessly
    (no re-encode, no quality loss) and is required for every HLS-sourced
    stream muxed into MP4 here.
    """
