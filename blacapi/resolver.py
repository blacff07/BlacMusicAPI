...
from blacapi import stats, webhook
...
_url_cache = TTLCache(ttl=settings.URL_CACHE_TTL, name="resolved_url")
_locks = KeyedLocks()
...
# Substrings yt-dlp/YouTube use across the various ways an invalid, expired,
# or flagged cookie session gets rejected. Deliberately broad (a false
# positive here just adds one extra log line, a false negative just means
# the operator has to notice a plain resolve failure instead) — this is
# operator-facing diagnostics, not a decision point.
_COOKIE_AUTH_FAILURE_MARKERS = (
    "sign in to confirm",
...
def _looks_like_cookie_auth_failure(exc: Exception) -> bool:
    msg = str(exc).lower()
    return any(m in msg for m in _COOKIE_AUTH_FAILURE_MARKERS)
    ...
    try:
        ...
        if settings.YT_COOKIES_FILE:
            ...
            try:
                ...
                last_exc = exc4
                if _looks_like_cookie_auth_failure(exc4):
                    # Distinct from a normal resolve failure — this is the
                    # cookie tier itself getting rejected, which almost
                    # always means YT_COOKIES has expired or was invalidated
                    # (YouTube flagging unusual activity on the account,
                    # etc.) rather than anything wrong with this video.
                    # Logged at warning level with a fixed, greppable prefix
                    # so it's obvious in Railway logs when it's time to
                    # export a fresh cookies.txt, instead of looking like a
                    # one-off failure on this particular video.
                    logger.warning(
                        f"[resolver] YT_COOKIES appears expired or invalid "
                        f"(cookie-tier resolve for {video_id} was rejected): {exc4}"
                    )

    classified = classify(last_exc)
    logger.error(f"[resolver] {video_id}: resolution failed ({classified.status_code}): {classified}")
    stats.note_resolve_failure(video_id)
    webhook.notify_resolve_failure(video_id, classified.status_code, str(classified))
    raise classified
...
# Rough abr (kbps) buckets for the ?quality= selector on /api/youtube/audio.
# Boundaries picked around common YouTube audio itags: itag 139 (~48kbps),
# itag 140 (~128kbps), itag 141/251 (~160kbps+) — "low" favors the smallest
# download for constrained hosts, "high" favors the best available.
_QUALITY_BUCKETS = {
    "low": lambda abr: abr is not None and abr <= 64,
    "medium": lambda abr: abr is not None and 64 < abr <= 128,
    "high": lambda abr: abr is not None and abr > 128,
}


async def list_audio_formats(video_id: str, limit: int = 8, quality: Optional[str] = None) -> list[dict]:
    ...
    ``quality`` (low/medium/high) reorders this list so a matching format
    comes first — it's a preference, not a hard filter, so a request for
    "low" on a video that only has one 128kbps format still gets that
    format rather than an empty list.
    """
    info = await _extract(video_id, "bestaudio/best")
    out = []
    ...
    if quality in _QUALITY_BUCKETS:
        matcher = _QUALITY_BUCKETS[quality]
        matched = [f for f in out if matcher(f.get("abr"))]
        if matched:
            rest = [f for f in out if f not in matched]
            out = matched + rest
