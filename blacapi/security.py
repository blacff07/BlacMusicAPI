...
async def verify_api_key(
    ...
    limit = settings.KEY_QUOTAS.get(key)
    if limit is not None and not quotas.check_and_increment(key, limit):
        raise HTTPException(
            status_code=429,
            detail=f"Daily quota exceeded for this API key (limit: {limit}/day)",
        )
